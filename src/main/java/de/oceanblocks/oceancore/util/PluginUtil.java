...
public class PluginUtil {
    ...
    public static String load(String name) {
        ...
        if (!pluginDir.isDirectory()) {
            return "Loading plugins from target directory";
        }
        ...
        if (!pluginFile.isFile()) {
            for (File f : pluginDir.listFiles()) {
                if (f.getName().endsWith(".jar")) {
                    try {
                        PluginDescriptionFile desc = Oceancore.getInstance().getPluginLoader().getPluginDescription(f);
                        if (desc.getName().equalsIgnoreCase(name)) {
                            pluginFile = f;
                            break;
                        }
                    } catch (InvalidDescriptionException e) {
                        return "Failed to load target plugin due to cannot find target plugin";
                    }
                }
            }
        }
        ...
        try {
            ...
            e.printStackTrace();
            return "Failed to load target plugin due to missing description";
        } catch (InvalidPluginException e) {
            e.printStackTrace();
            return "Failed to load target plugin due to invalid plugin";
        }

        target.onLoad();
        Bukkit.getPluginManager().enablePlugin(target);

        return "Successfully loaded " + target.getName();
    ...
    public static String unload(Plugin plugin) {
        ...
        return "Successfully unloaded " + name;
